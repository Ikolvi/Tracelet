...
**FIX**: `TraceletSdk.startBackgroundActivitySessionIfNeeded()` — every moving transition and the in-app-evaluated geofence posture — now honours `useBackgroundActivitySession`, opening the session only with the opt-in or under When-In-Use authorization, where it is what lets the stream survive suspension. Under Always it only ever added the indicator ([#423](https://github.com/Ikolvi/Tracelet/issues/423)).

**FIX**: `LocationEngine` arms a silence watchdog when the continuous stream starts and re-arms it in `didUpdateLocations`, so a stream delivering nothing announces itself on its own schedule instead of waiting for a fix that never comes. `noteFilterDecision` is reached only from the delegate callback, so the stall watchdog added in #397 could not see total silence on either platform — this is the iOS half of the Android fix in the same release. New `silenceAnnounceSeconds` (45, against the stall watchdog's 120 — silence is the harder failure, and no threshold change recovers it), `silencePollSeconds` (15), and a `lastCallbackAt` clock deliberately separate from `lastAcceptedFixAt`: one tracks CoreLocation, the other tracks the filter, and sharing them is the bug. The clock is `ProcessInfo.systemUptime`, not `Date()`, so an NTP step cannot announce a silence that never happened or hide a real one. Cancelled in `stop()`, `startPeriodic()` and `announceContinuousStop(becoming:)`, and re-armed by `announceContinuousStart(resuming:)`, so a stationary park is not reported as a fault and a resume out of one is covered again. `evaluateSilence(atUptime:)` and `noteCallbackDelivered(atUptime:)` take the clock as a parameter so the 45-second window can be tested without waiting it out ([#407](https://github.com/Ikolvi/Tracelet/issues/407), [#397](https://github.com/Ikolvi/Tracelet/issues/397), [#318](https://github.com/Ikolvi/Tracelet/issues/318)).

**FIX**: `TraceletSmartMotionCoordinator.resolvedSpeed` applies `LocationEngine.maximumPaceFixAge` to the fix behind `lastEffectiveSpeed`, returning `nil` past it so the tremor override takes the existing `.none` branch rather than overruling the accelerometer — the same gap as Android, found in an Android field trace. New `LocationEngine.paceFixAge` reports the age from `location.timestamp`, and `maximumPaceFixAge` drops `private` so the override and the pace sink cannot drift apart. The decline is announced once per run on the lifecycle channel, guarded by the new `declinedStaleTremorSpeed` ([#404](https://github.com/Ikolvi/Tracelet/issues/404), [#318](https://github.com/Ikolvi/Tracelet/issues/318)).

**FIX**: `SpeedMotionManager.stop()` calls `stopSlowingTimer()`, and the SLOWING `DispatchWorkItem` guards on `isRunning` as well as `state` — the same defect as Android, found in an Android field trace. A scheduled item cannot be recalled once it fires, only ignored, so a session stopped mid-SLOWING drove `switchToStationary()` minutes later and armed the stationary schedule on a stopped session ([#412](https://github.com/Ikolvi/Tracelet/issues/412)).

**FIX**: `TraceletSdk.start()`'s stationary branch starts the engine when `locationEngine.geofenceHighAccuracyMode` is set, mirroring Android. `wireGeofenceLocationCallbacks()` runs before `locationEngine.start()` and already assigns `geofenceHighAccuracyMode = geofenceManager.hasEvaluatorOwnedGeofences()`, so for a fence stored before the session `applyGeofenceEvaluationCadence()`'s `guard locationEngine.geofenceHighAccuracyMode != needsInAppEvaluation else { return }` saw no change and returned — leaving the flag set, the `distanceFilter` at 0, and no stream open for the evaluator to decide from. `isEvaluatorOwned` is identical on both platforms (polygons, sub-100 m circles, or `geofenceModeHighAccuracy`), so a 10 m circle is evaluator-owned on iOS too; only the start-time branch was missing. The reconcile that follows parks the stream again when both motion inputs say stationary ([#412](https://github.com/Ikolvi/Tracelet/issues/412), [#357](https://github.com/Ikolvi/Tracelet/issues/357)).

**FIX**: `TraceletSdk.startSpeedMotionManager` applies the last-known-speed seed only when the pace was inherited — `isResume`, or `forceMoving` — through the new `TraceletSdk.paceWasInherited(isResume:forceMoving:)`. The seed can only wake the machine, and the `!forceMoving` block below reads the woken state back into `stateManager.isMoving`, so a fresh `start()` that committed `motion.isMoving: false` came up `.moving` off a stale fix — and because a fresh process has no fix to seed with, the first `start()` and the second returned different paces for the same call. `SpeedMotionManager.start(forceMoving:)`'s forced-moving line moves from `debug` to `lifecycle`, mirroring `speed-motion: restored` ([#344](https://github.com/Ikolvi/Tracelet/issues/344), [#318](https://github.com/Ikolvi/Tracelet/issues/318)). The Android posture fix in #412/#414 does not apply: `switchToStationaryPeriodicForce()` leaves `trackingMode` on `.continuous`, so the reconcile always takes the branch that reads `isContinuousStreaming`.

**FIX**: `TraceletSmartMotionCoordinator.onSpeedStateChange(isMoving:)` routes its tremor override through `onAccelStateChange(isMoving:)`, so the action that override produces is handled and returned rather than dropped by `_ =` — with `isSpeedMoving` already false it is the transition that carries the stop, and the core call behind it dedupes to `.none`. `TraceletSdk.startSpeedMotionManager` seeds the coordinator's speed input on a forced-moving start, which only the non-forced branch used to write. `LocationEngine` gains `isContinuousStreaming`: `isTracking` stays true on an engine parked by `switchToStationaryPeriodic()`/`switchToStationaryGeofences()`, so `syncCurrentMode()`'s `isStreamLive` input was answering "a session is alive" rather than "the stream is running", and both park methods plus `switchToContinuous()` now write the same always-on `location stream: continuous updates …` lines `start()`/`stop()` do. Those two ends are paired: `stop()` announces a stop only when updates were flowing, and `start()` announces a start only when it actually starts them — a session in `useSignificantChangesOnly` or low-power geofence mode records `continuous updates skipped` and the reason instead of claiming a stream it deliberately never opens. New `TraceletSmartMotionCoordinator.reconcilePosture()` re-reads the posture and calls `evaluateConfigurationChange(useGeofences:)`, and `start()` runs it after the pace branch: `syncCurrentMode()` alone can only read the committed pace, so anything that opens a stream afterwards — the #357 in-app fence evaluator among them — left a parked coordinator over a live stream that no later input could end, since `evaluate_state` answers a repeated input with `.none` — and the same reconcile runs behind the mid-session branch that opens a stream for a newly added fence ([#412](https://github.com/Ikolvi/Tracelet/issues/412)). On a resume into a live process that seed is taken from `smm.state` rather than from `forceMoving`, because `SpeedMotionManager.start()` returns early when it is already running — asserting "moving" there held the stream open over a pace machine that had already stood the session down. `reconcilePosture()` records its decision on the always-on lifecycle channel, naming both motion inputs and the engine. iOS is unaffected by the geofence half of [#414](https://github.com/Ikolvi/Tracelet/issues/414): `GeofenceManager`'s evaluator wake-up resumes the stream without writing the pace ([#412](https://github.com/Ikolvi/Tracelet/issues/412), [#414](https://github.com/Ikolvi/Tracelet/issues/414)).
