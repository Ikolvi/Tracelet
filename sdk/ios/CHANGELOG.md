...
**FIX**: `TraceletSdk.startSpeedMotionManager` seeds the coordinator's speed input from `smm.state` rather than from `forceMoving`: `SpeedMotionManager.start()` returns early when it is already running, so on a resume into a live process the machine keeps whatever state it had and asserting "moving" held the stream open over a pace machine that had already stood the session down. `TraceletSmartMotionCoordinator.reconcilePosture()` records its decision on the always-on lifecycle channel, naming both motion inputs and the engine. iOS is unaffected by the geofence half of ([#414](https://github.com/Ikolvi/Tracelet/issues/414)): `GeofenceManager`'s evaluator wake-up resumes the stream without writing the pace ([#414](https://github.com/Ikolvi/Tracelet/issues/414))
