...
**FEAT**: geofences smaller than CoreLocation can resolve are now supported rather than merely warned about. Region monitoring needs a radius of roughly 100 m — below that the fence is smaller than the error of the fixes it is compared against, so it never becomes confident enough to report a crossing (#355). A fence under that radius is now owned by the in-app evaluator instead: it is decided against its *true* radius, the monitored region is registered at 100 m purely as a wake-up, and the OS's own transitions for it are discarded because they describe the wrong boundary. Three changes make that decidable. The exit-hysteresis band now tracks the **measured** fix accuracy — clamped to 3–20 m — instead of a flat 20 m floor, so a 10 m fence on a 4 m-accurate handset needs ~8 m of travel to EXIT rather than ~28 m; the flat floor is kept only for fixes that report no accuracy at all, where there is nothing to measure. Repeated fixes of the same spot are combined by inverse-variance weighting into a tighter estimate (`accuracy / sqrt(n)`), discarded the moment the device demonstrably moves so the average never lags a walking user. And ownership is decided per fence, so a 20 m fence and a 500 m fence in one config are each handled by whichever component can actually resolve them. **Polygons are covered by the same change**: CoreLocation only monitors circular regions, so a polygon was always the SDK's to evaluate, but it was only ever evaluated when `geofenceModeHighAccuracy` happened to be on — at default settings a polygon geofence silently never fired. Note the cost: a fence the OS cannot serve needs the location stream running, which an OS-resolvable fence does not ([#356](https://github.com/Ikolvi/Tracelet/issues/356)).
