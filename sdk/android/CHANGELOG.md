...
**FIX**: `LocationEngine.collectSamples()` runs a `requestLocationUpdates()` request beside the `getCurrentLocation()` loop, counts a fix delivered by both once, and unregisters on every exit — enough samples, timeout, or `SecurityException`. The #46 comment claiming it already used `requestLocationUpdates()` was never true ([#416](https://github.com/Ikolvi/Tracelet/issues/416)).

**FIX**: `SmartMotionCoordinator.resolvedSpeed` applies `LocationEngine.MAX_PACE_FIX_AGE_MS` to the fix behind `lastEffectiveSpeed`, returning `null` past it so the tremor override takes the existing "unresolved speed" branch instead of overruling the accelerometer. The override read a stored value with no age gate at all, while the pace sink in `LocationEngine` has enforced the same constant since #385 — the `#333` reasoning above it covers a speed being *absent* but never its being *old*, and the two fail in opposite directions: `null` leaves the accelerometer standing, a stale near-zero actively overrules it. New `LocationEngine.paceFixAgeMs` reports the age off `elapsedRealtimeNanos`, the same monotonic source the sink's own gate uses, and `MAX_PACE_FIX_AGE_MS` becomes `internal` so one constant governs both. The decline is announced once per run on the lifecycle channel, guarded by the new `declinedStaleTremorSpeed`, mirroring `staleFixesSincePace` ([#404](https://github.com/Ikolvi/Tracelet/issues/404), [#318](https://github.com/Ikolvi/Tracelet/issues/318)).

**FIX**: `LocationService.startForegroundWithNotification()` checks `BackgroundRestrictions.isBackgroundRestricted()` before recording the outcome, and records `result = "refused", promoted = false` with a `BackgroundRestriction` failure class instead of `success` when the app is in the Restricted battery state. `startForeground()` neither throws nor returns a value there — the OS logs `not allowed due to bg restriction` and demotes the record — so the previous `recordPromotion("success", promoted = true)` on the line after the call was unconditional. `foregroundServiceHealth()` therefore now reports `serviceForeground = false` and a null `foregroundNotificationId` in that state, and the new `announceRefusedPromotion()` names it once per record on the lifecycle channel, alongside `announceLocationBlindPromotion()` (#405) which covers the promotion that took effect but cannot use location ([#406](https://github.com/Ikolvi/Tracelet/issues/406)).

**FIX**: `SpeedMotionManager.stop()` calls `stopSlowingTimer()`, and the posted SLOWING runnable guards on `started` as well as `currentState`. `stop()` cleared `started` but left the countdown armed, and a posted `Handler` callback cannot be recalled — only ignored — so a session stopped mid-SLOWING ran `transitionTo(STATIONARY)` + `switchToStationary()` later, driving `SmartMotionCoordinator` into SWITCH_TO_STATIONARY_PERIODIC and `LocationService.switchToStationaryPeriodic()`, which writes `trackingMode = PERIODIC`, writes `isMoving` and arms the stationary `Handler` timer on a stopped session. `LocationEngine.onLocationReceived` now returns early when `state.enabled` is false, guarded by the new `reportedFixWhileDisabled` (reset in `start()`): `fusedClient.removeLocationUpdates` is asynchronous, so an in-flight fix still reached the full pipeline after `stop()`. `state.enabled` is set in `start()` before the engine starts or `requestStartupFix()` runs, and `getCurrentPosition()` never routes through `onLocationReceived`, so the one-shots that legitimately deliver without a continuous stream are unaffected ([#412](https://github.com/Ikolvi/Tracelet/issues/412)).

**FIX**: `LocationEngine.onLocationReceived` announces a fix delivered while `trackingCallback` is null on the lifecycle channel, guarded by the new `reportedFixWhileStopped` so it fires once per stopped period. `stop()` removes `trackingCallback` and logs `continuous updates stopping`, so fixes continuing after a park come from somewhere else; the entry carries `event`, `System.identityHashCode(this)`, `isPeriodicTracking` and `watchers.size` to name which. `start()`/`stop()` maintain a process-wide `LiveStreams` count and both lifecycle entries carry `engine=<identityHashCode>` and `liveStreams=<n>`: every registration inside one engine re-uses the single `trackingCallback` (`updateLocationProviderOptions`, `reapplyProviderOptionsIfTracking`, `activateGpsFallback` and `restoreOriginalPriority` all replace the request in place), so one engine cannot leak a second stream and `liveStreams>1` is the only reading that names a second engine. Note `watchers` deliberately survives `stop()` — a `watchPosition` caller owns its own `stopWatchPosition`, and a stationary park must not silently cancel it ([#412](https://github.com/Ikolvi/Tracelet/issues/412), [#318](https://github.com/Ikolvi/Tracelet/issues/318)).

**FIX**: `MotionDetector` arms the stop-timeout countdown on `consecutiveStillSamples >= stillCount` while none is pending, rather than on `== stillCount`. `handleActivityTransition` calls `cancelStopTimeout()` on every ENTER of a moving activity but only resets the streak through `declareMoving()`, which it skips when the pace is already moving — so sustained walking disarmed the countdown permanently, the streak being monotonic. `stopTimeoutRunnable` is cleared when it fires and is now `@Volatile`, since the stillness sampler reads it from the sensor thread. `declareStationary()` guards on the new detector-owned `selfReportedMoving` instead of `state.isMoving` when `motionDetectionMode` is SMART: the method already refuses to *write* that field because `SmartMotionCoordinator` owns it, but reading it made the same ownership claim in reverse, and `SpeedMotionManager.transitionTo()` sets `state.isMoving = false` on reaching STATIONARY — so the accelerometer's edge was swallowed before `onMotionStateChanged` was invoked, leaving `is_accel_moving` true with no path back. iOS is unaffected: its `commitTransition` does not write the shared flag, and `startStopTimeoutCountdown` already re-arms from the activity-update path. The tremor-override decline in `SmartMotionCoordinator.onSpeedStateChange` moves from `info` to `lifecycle` ([#412](https://github.com/Ikolvi/Tracelet/issues/412), [#318](https://github.com/Ikolvi/Tracelet/issues/318)).

**FIX**: `TraceletSdk.start()` applies the last-known-speed seed only when the pace was inherited — a resume, or `shouldForceMoving` — through the new `TraceletSdk.paceWasInherited(isResume:forceMoving:)`. The seed can only wake the machine, and the wake is read back into `stateManager.isMoving`, so a fresh `start()` that committed `motion.isMoving: false` came up moving off a stale fix; the first `start()` of a process and the second therefore disagreed. `SpeedMotionManager.start()`'s forced-moving line moves from `debug` to `lifecycle`, mirroring `speed-motion: restored` ([#344](https://github.com/Ikolvi/Tracelet/issues/344), [#318](https://github.com/Ikolvi/Tracelet/issues/318)).

**FIX**: `GeofenceManager.onEvaluatorWakeup` runs the new `LocationService.resumeStreamForEvaluator()` instead of `switchToContinuous()`. The latter is a motion transition — `state.isMoving = true`, `trackingMode = CONTINUOUS`, `stopStationaryTimer()` — so a fence hint committed a pace no sensor had asked for, which `start(isResume = true)` then read as `shouldForceMoving` on every later resume. The new path starts the engine, leaves the pace alone, and arms a 60 s window after which `SmartMotionCoordinator.reconcilePosture()` decides the cadence again. `TraceletSdk.start()`'s SMART branch seeds the coordinator's speed input from `speedMotionManager.getCurrentState()` rather than from `shouldForceMoving`, because `SpeedMotionManager.start()` early-returns (`if (started) return`) when the manager is already running. `reconcilePosture()` and the boot-mode continuous switch move from `debug` to `lifecycle` ([#414](https://github.com/Ikolvi/Tracelet/issues/414))

**FIX**: `SmartMotionCoordinator.onSpeedStateChange` handles the action its tremor override produces instead of leaving the return of `coreCoordinator.onAccelStateChange(false)` unread. With `isSpeedMoving` already false that override *is* the transition `evaluate_state` answers with SWITCH_TO_STATIONARY_PERIODIC, and the `onSpeedStateChange` behind it dedupes to NONE, so the engine was never switched. `TraceletSdk.start()`'s SMART branch now seeds `smartMotionCoordinator.onSpeedStateChange(true)` on a forced-moving start: `adoptSpeedMotionPace()` runs only on the non-forced path, so the FFI-side `is_speed_moving` kept a previous session's stationary value for the life of the process. New `SmartMotionCoordinator.reconcilePosture()` re-reads the posture and calls `evaluateConfigurationChange`, and `TraceletSdk.start()` runs it after the pace branch: `syncCurrentMode()` alone can only read the committed pace, so the `geofenceHighAccuracyMode` branch that starts a stream for an in-app-evaluated fence (#357) left a parked coordinator over a live one — and `evaluate_state` answers a repeated input with NONE, so nothing the session could say would ever end that stream — and the same reconcile runs behind the mid-session branch that opens a stream for a newly added fence ([#412](https://github.com/Ikolvi/Tracelet/issues/412)).

**FIX**: `SmartMotionCoordinator.syncCurrentMode()` maps the coordinator posture from `stateManager.isMoving || locationEngine.isTracking` rather than the committed pace alone, so a start that inherits a live stream still reaches the stationary switch ([#409](https://github.com/Ikolvi/Tracelet/issues/409)). The `TrackingMode.PERIODIC` branch reads `locationEngine.isTracking` too, rather than answering `STATIONARY_PERIODIC` unconditionally: `LocationService.switchToStationaryPeriodic()` writes `state.trackingMode = PERIODIC`, so every reconcile *after* the first park took that branch instead of the `CONTINUOUS` one, and `evaluate_state` emits the stop only from `Continuous` — so `reconcilePosture()` returned NONE for both callers that open a stream behind the coordinator, `TraceletSdk.applyGeofenceEvaluationCadence()` (#357) and `LocationService.resumeStreamForEvaluator()` (#414), each of which runs after that first park in a real session. `isTracking` is `trackingCallback != null`, which `startPeriodic()` never registers, so a genuine periodic session still reads as stationary ([#412](https://github.com/Ikolvi/Tracelet/issues/412), [#414](https://github.com/Ikolvi/Tracelet/issues/414)). `LocationService.startBootTracking()` reads the new `TraceletSdk.hasLiveSessionEngine` — narrower than `isTracking`, which is unconditionally true from inside the service — before `bootstrapForBackground()` can re-initialise and answer it wrongly, and returns without building a second engine when a session engine is already live in the process ([#410](https://github.com/Ikolvi/Tracelet/issues/410)).
