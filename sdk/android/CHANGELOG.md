...
**FIX**: a circular geofence whose radius the platform cannot service now says so instead of failing silently. Play Services' guidance is a radius of at least 100 m; below that the fence is smaller than the location error it is compared against, and the in-app evaluator cannot help either — EXIT needs `radius + max(radius * 0.1, 20 m)` of separation, so a 5 m fence requires ~25 m of travel before it can fire. The failure is deceptive rather than obvious: registering while inside fires an immediate ENTER from the initial trigger, so the fence looks live, and then no crossing is ever reported again — a field report of "ENTER fires once, EXIT never" came from exactly this. A warning is now written on the always-on lifecycle channel, so it reaches a release-build bug report. Small fences are still registered; the floor is advisory on both platforms ([#355](https://github.com/Ikolvi/Tracelet/issues/355)).
