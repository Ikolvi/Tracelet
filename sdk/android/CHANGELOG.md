...
**FIX**: `SmartMotionCoordinator.syncCurrentMode()`'s `TrackingMode.PERIODIC` branch reads `locationEngine.isTracking` instead of answering `STATIONARY_PERIODIC` unconditionally. `LocationService.switchToStationaryPeriodic()` writes `state.trackingMode = PERIODIC`, so every reconcile *after* the first park took that branch rather than the `CONTINUOUS` one the #409/#412 fix added the OR to — and `evaluate_state` emits the stop only from `Continuous`, so `reconcilePosture()` returned NONE. Both callers that open a stream behind the coordinator run after that first park: `TraceletSdk.applyGeofenceEvaluationCadence()` (#357) and `LocationService.resumeStreamForEvaluator()` (#414). `isTracking` is `trackingCallback != null`, which `startPeriodic()` never registers, so a genuine periodic session still reads as stationary ([#412](https://github.com/Ikolvi/Tracelet/issues/412), [#414](https://github.com/Ikolvi/Tracelet/issues/414)).
