...
**FIX**: a circular geofence whose radius the platform cannot resolve now records how it will be handled instead of failing silently. Registering while inside fires an immediate ENTER from the initial trigger, so the fence looks live, and then no crossing is ever reported again — a field report of "ENTER fires once, EXIT never" came from exactly this. A note is now written on the always-on lifecycle channel, so it reaches a release-build bug report. Superseded in part by #356, which makes these fences work rather than merely explain themselves ([#355](https://github.com/Ikolvi/Tracelet/issues/355)).
