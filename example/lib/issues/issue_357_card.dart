    ...
    try {
      ...
      check(
        ...
        '$windowB raw fixes in ${_windowSeconds}s after the fence was added '
            '(baseline $windowA). Before this fix the flag was only computed at '
            'start(), so this window read the same as the baseline and the '
            'evaluator saw one fix per distanceFilter metres travelled',
      );
