...
**FIX**: the four log-derived sections share one read instead of fetching the same rows over the platform channel four times, and the lifecycle and geofence scans default to 5000 entries rather than 2000. 2000 was also the SDK's row cap at `logLevel: verbose`, so the scan could see the whole table and no more; now that the cap is applied per channel, the always-on `LIFECYCLE` rows are kept *behind* up to a full cap of level-based chatter and a 2000-row scan would stop short of exactly the entries these sections exist to surface ([#318](https://github.com/Ikolvi/Tracelet/issues/318)).
