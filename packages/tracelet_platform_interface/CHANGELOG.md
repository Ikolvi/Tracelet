...
**FIX**: the `flutter_rust_bridge` runtime is pinned to the codegen version — `>=2.13.0 <2.14.0`, with the bindings regenerated by 2.13.0 — so `RustLib.init()` no longer fails with a codegen/runtime version mismatch on a fresh `pub get` ([#421](https://github.com/Ikolvi/Tracelet/issues/421)).
