...
**FIX**: (Android) a fix arriving while continuous updates are stopped is recorded on the always-on lifecycle channel. A parked session was still recording roughly a fix a second for the whole stationary window, with no `continuous updates starting` entry between the park and the next wake-up — so the callback the park removed cannot have been the source, and the trace had no way to say what was. The entry names the caller, the engine identity and whether a periodic tick or a position watch is outstanding, which separates a second registration on one engine from a second engine in the same process. It is emitted once per stopped period rather than per fix. The stream start/stop entries now carry the same engine identity plus a count of how many engines in the process hold a live stream, because a second engine's own callback is not null and so could never have tripped the first check ([#412](https://github.com/Ikolvi/Tracelet/issues/412), [#318](https://github.com/Ikolvi/Tracelet/issues/318)).
...
**FIX**: (Android) a session that has already parked once no longer holds a full-rate stream open for the rest of its life. #412 and #414 both fixed a stream opened behind the motion coordinator by asking it to re-judge the state the session is in — but on Android the *park itself* writes `trackingMode = periodic` (iOS deliberately leaves it alone), and that reading answered "already stationary" without ever looking at the engine. The core only emits the stop from a continuous posture, so the reconcile returned nothing and the stream survived it. Both routes that open a stream behind the coordinator run *after* the first park in a real session — a fence added from the map, and an OS wake-up near an in-app fence — so what the device shows is a phone reporting `Is moving: false` in `periodic` mode while delivering a fix every two seconds, with the location indicator lit until tracking is stopped. The posture is now read from the live stream in that mode too ([#412](https://github.com/Ikolvi/Tracelet/issues/412), [#414](https://github.com/Ikolvi/Tracelet/issues/414)).
...
**FIX**: (Android, iOS) a session that starts stationary no longer holds a full-rate GPS stream open for its whole life. A fence too small for the OS to resolve is decided in-app, from the location stream — so when one exists, a stationary start opens a stream it otherwise would not (#357). The motion coordinator was told the session's *pace* before any of that happened, and it only ever acts on a **change**: with both motion inputs already stationary, every later reading was a repeat and was discarded, so nothing the session could say would ever end that stream. The reported device ran 2-second fixes at full accuracy, every one of them `is_moving: false`, from the moment Start was pressed until Stop was — with the location indicator lit throughout, which is exactly what it was reporting. The coordinator is now reconciled once the engine has settled, so it judges the state the session is actually in instead of the one predicted before it began: a stationary session that inherited a stream is parked onto the stationary schedule — where the fence is still evaluated, at the stationary cadence rather than at full rate — and a moving one keeps streaming. The same reconcile runs when a fence is added mid-session to a parked one, which opens a stream by the same route ([#412](https://github.com/Ikolvi/Tracelet/issues/412)).
