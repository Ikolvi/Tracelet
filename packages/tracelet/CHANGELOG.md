...
**FIX**: (Android) `getCurrentPosition()` now wakes the provider for a fresh fix instead of retrying a call that can answer from cache forever. The 1.8.7 fix for #46 said `getCurrentPosition(samples: 1)` had moved to `requestLocationUpdates()` so it would wake GPS and return a fresh position; the code never did — every sample came from a one-shot `getCurrentLocation()`, retried every 800 ms, which can return null or a cached fix without engaging the hardware, and on the reporting device did exactly that until the timeout. Samples are now collected from two sources side by side for the bounded window: a continuous request, which is the only API that actually wakes the provider, and the one-shot loop, kept because it is the path that answers on budget devices whose battery optimisation throttles continuous requests for an app without a foreground service. The first genuine fix from either counts, a fix both deliver is counted once so `samples` still means distinct fixes, and every exit — enough samples, timeout, permission failure — removes the continuous request so a one-shot operation never leaves the provider running ([#416](https://github.com/Ikolvi/Tracelet/issues/416)).

**FIX**: (Android) `requestLocationAuthorization()` no longer hangs forever when the Activity is detached while the permission dialog is showing. The plugin completes the pending reply only from `onRequestPermissionsResult`, delivered through the listener it registers on the `ActivityPluginBinding`; `onDetachedFromActivity()` removed that listener and, since the 2.0.0 lifecycle rewrite, did nothing about the reply — so a dialog answered after a detach left the Future unresolved, and the *next* call short-circuited on the stale pending callback and returned the current status synchronously, which is why it appeared to work on the second try. The plugin now completes the reply with the current status on a full detach, before the activity is dropped so DENIED can still be told from NOT_DETERMINED, and leaves it alone on the config-change detach where the listener is re-added on reattach and the result still arrives ([#415](https://github.com/Ikolvi/Tracelet/issues/415)).

**FIX**: (iOS) continuous tracking no longer shows the blue location bar on an Always-authorized app that opted out of it. `IosConfig.useBackgroundActivitySession` is documented as opt-in and defaults to `false`, but only `LocationEngine.start()` ever read it: every transition to *moving* went through a path that checked `useSignificantChangesOnly` and nothing else, and opened a `CLBackgroundActivitySession` — which shows the system indicator for as long as it exists, regardless of `showsBackgroundLocationIndicator`. So the exact configuration the #210 resolution told users to adopt to hide the indicator showed it for the length of every trip, and it went away on its own some minutes after the device parked, which is what made it look like an OS quirk rather than the SDK. Under Always the session buys nothing — `allowsBackgroundLocationUpdates` and the `location` background mode already keep the stream delivering — so it now opens only with the opt-in. The one exception is When-In-Use authorization, where the session is what lets the stream survive suspension and the OS shows the indicator regardless; there it still opens, so no app that relied on it loses tracking. The in-app-evaluated geofence posture, which had the same unconditional start, goes through the same gate ([#423](https://github.com/Ikolvi/Tracelet/issues/423)).

**FIX**: (Android, iOS) `Tracelet.ready()` no longer throws `Bad state: tracelet_core's codegen version (2.12.0) should be the same as runtime version (2.13.0)` on a fresh install. The bindings were generated by `flutter_rust_bridge` 2.12.0 but the runtime was constrained as `^2.12.0`, and `flutter_rust_bridge` checks the two match exactly at init. Since 2.13.0 was published on 23 August every new `pub get` resolved it, so every new integration of 3.8.7 crashed at `ready()`. The bindings are now generated by 2.13.0 and the runtime is pinned to the same minor, `>=2.13.0 <2.14.0`, so pub cannot resolve a runtime the generated code rejects; CI installs the codegen at that exact version too, instead of whatever is latest, so the two cannot drift again ([#421](https://github.com/Ikolvi/Tracelet/issues/421)).
