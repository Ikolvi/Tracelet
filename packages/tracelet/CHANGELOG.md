...
**FIX**: (Android, iOS) a device that has been sitting still no longer discards the movement that should wake it. When the GPS-speed machine reports stationary and the accelerometer disagrees, the SDK settles it with the last resolved speed — near zero means the accelerometer is reading hand tremor on a still device, and it is overruled. That reading had no freshness check, while the pace machine beside it has refused fixes older than ten seconds since 3.8.7, so the two disagreed about a number neither could vouch for. A parked device stops its stream and the reading freezes at whatever it last resolved — 0.0057 m/s in the reported trace, well under the tremor threshold — and the stationary schedule produces no fresh fix *by construction*, which is exactly when the frozen value is most likely to be wrong. So a walk started minutes later, with the app in the background, was overruled by a speed measured before the device stopped, and the session never left stationary. Reopening the app produced a fresh fix and unwedged it, which is why this was reported as "the location indicator only appears while the app is in the foreground". An old reading is now *unknown* in the same sense a missing one already was — and unknown leaves the accelerometer standing rather than siding against it — with the decline recorded on the always-on channel ([#404](https://github.com/Ikolvi/Tracelet/issues/404), [#344](https://github.com/Ikolvi/Tracelet/issues/344), [#318](https://github.com/Ikolvi/Tracelet/issues/318)).

**FIX**: (Doctor) a bug report no longer says "all systems healthy" on a device whose OS has blocked background tracking. The blockers — the Restricted battery state, the RESTRICTED standby bucket, a foreground service denied the location capability, and a promotion the OS refused — are listed in the health verdict at the top of the report rather than only in a section further down that a reader has already been told to skip ([#406](https://github.com/Ikolvi/Tracelet/issues/406), [#405](https://github.com/Ikolvi/Tracelet/issues/405)).

**FIX**: (Android) a foreground-service promotion the OS refuses is no longer reported as a successful one. In the "Restricted" battery state Android refuses the promotion and says so only in its own log — `Service.startForeground() not allowed due to bg restriction` — without throwing, so the call returned normally and the SDK recorded success. A device in that state showed `isForeground=false types=00000000 caps=------` — demoted, with no location capability, GPS switched off about a second after every registration — while the report read `Promoted to foreground: true / Last promotion result: success` for a session that could not record anything in the background. A health snapshot that reads green while the OS has switched tracking off sends every investigation into the SDK instead of at the restriction, so the refusal is now reported as its own outcome and announced on the always-on channel ([#406](https://github.com/Ikolvi/Tracelet/issues/406), [#405](https://github.com/Ikolvi/Tracelet/issues/405)).

**FIX**: (Android, iOS) stopping tracking no longer leaves a countdown that restarts it. The GPS-speed machine's stationary countdown is a scheduled callback, and `stop()` cleared the running flag without cancelling it — while the countdown's own guard tested the motion state, not whether the session was still alive. A session stopped part-way through slowing down therefore transitioned to stationary minutes later, and that transition is not inert: it switches the coordinator into the stationary schedule, which rewrites the tracking mode and the pace and arms a repeating location timer. A field report shows the stop at 18:43:09 and the countdown expiring at 18:45:59, leaving a stopped app reporting `Tracking mode: periodic`. Separately, because the platform's "stop location updates" call is asynchronous, a fix already in the provider's delivery pipeline still arrived afterwards and was run through the whole pipeline — enriched, geocoded, added to the odometer and written to the database the user had just stopped filling. Such a fix is now dropped, and the drop is recorded once on the always-on channel ([#412](https://github.com/Ikolvi/Tracelet/issues/412)).

**FIX**: (iOS) a geofence too small for the OS to resolve is now evaluated when the session starts stationary. Such a fence is decided from the location stream rather than by CoreLocation, and a stationary start deliberately runs no stream — so a fence stored *before* `start()` was never evaluated at all, staying dead until the device happened to move. The re-alignment that covers a fence added mid-session could not cover this one: it returns early unless ownership changed, and start-up had already set the flag from the stored fences a moment earlier, so the flag was right and nothing ever opened the stream it implied. Android has opened that stream since the original fix; iOS never did. The stream is opened and then judged like any other a stationary session inherits — parked back to the stationary schedule when both motion inputs agree, which still feeds the evaluator, rather than held open at full rate ([#412](https://github.com/Ikolvi/Tracelet/issues/412), [#357](https://github.com/Ikolvi/Tracelet/issues/357)).

**FIX**: (Android) a fix arriving while continuous updates are stopped is recorded on the always-on lifecycle channel. A parked session was still recording roughly a fix a second for the whole stationary window, with no `continuous updates starting` entry between the park and the next wake-up — so the callback the park removed cannot have been the source, and the trace had no way to say what was. The entry names the caller, the engine identity and whether a periodic tick or a position watch is outstanding, which separates a second registration on one engine from a second engine in the same process. It is emitted once per stopped period rather than per fix. The stream start/stop entries carry the same engine identity plus a count of how many engines in the process hold a live stream, which is the only reading that separates a second engine from a second registration on one ([#412](https://github.com/Ikolvi/Tracelet/issues/412), [#318](https://github.com/Ikolvi/Tracelet/issues/318)).

**FIX**: (Android) walking for a while and then standing still no longer leaves the location indicator lit for the rest of the session. Stopping after a *brief* move parked correctly, which is what made this look intermittent: the difference is that a fix taken seconds after a real walk still reads well above the near-zero speed the coordinator treats as hand tremor, so it declines to overrule the accelerometer and waits for the accelerometer to stand down on its own — and on Android two separate faults meant it never could. The stop countdown was armed on the single sample where the stillness streak *crossed* its threshold, and a walking activity update cancels that countdown without resetting the streak; since the streak only grows, it could never cross again. And the accelerometer's own "am I already stationary?" check read the shared `isMoving` flag, which in smart mode is the *combined* answer and which the GPS-speed machine writes the moment its own countdown expires — so the speed machine going stationary first silenced the accelerometer's edge before it was ever delivered. With both inputs mute the combined answer stayed "moving" with nothing left to change it, and full-rate GPS ran on a phone sitting still ([#412](https://github.com/Ikolvi/Tracelet/issues/412)).

**FIX**: (Android, iOS) a session that is not moving no longer holds a full-rate GPS stream open for the rest of its life. The SMART coordinator holds a *posture* — is the engine streaming, or is it on a stationary schedule — and the core only emits the stop action when it believes that posture is continuous. That posture was read from the committed pace alone, so every route that puts a stream up behind the coordinator wrote "parked" over a live engine, and no later stationary decision produced anything: a `ready()` takeover with `isMoving = false` over a running stream ([#409](https://github.com/Ikolvi/Tracelet/issues/409)); a stationary start, which opens a stream it otherwise would not because a fence too small for the OS to resolve has to be decided from one ([#357](https://github.com/Ikolvi/Tracelet/issues/357)); a fence added to a parked session from the map; and an OS wake-up near an in-app fence ([#414](https://github.com/Ikolvi/Tracelet/issues/414)). On Android the park *itself* writes `trackingMode = periodic`, so after the first park the reconcile answered "already stationary" without looking at the engine at all — and that is the state a real session spends most of its life in. Reported devices held 2-second fixes at full accuracy for six unbroken minutes, and for whole sessions from Start to Stop, every fix `is_moving: false`, lying on a desk with the location indicator lit: the pace was right the whole time, only the stream was never told. The posture is now the OR of the committed pace and the engine's live state in every mode, and the coordinator is reconciled once the engine has settled rather than from a prediction made before it started — so a stationary session that opened or inherited a stream is parked onto the stationary schedule, where an in-app fence is still evaluated at the stationary cadence rather than at full rate, and a moving one keeps streaming ([#412](https://github.com/Ikolvi/Tracelet/issues/412), [#409](https://github.com/Ikolvi/Tracelet/issues/409), [#414](https://github.com/Ikolvi/Tracelet/issues/414), [#357](https://github.com/Ikolvi/Tracelet/issues/357)).

**FIX**: (Android, iOS) two calls to `start()` with the same configuration no longer give two different paces. `start()` seeds the GPS-speed machine with the last speed the process resolved — which exists for a *resume*, where feeding a fabricated 0 m/s stood a still-walking session down. The seed can only ever push the machine up, though, so a fresh `start()` that had just committed `motion.isMoving: false` came back *moving* on the strength of a stale reading. The first `start()` of a process has no fix to seed with and stays stationary as asked; the second seeds from the fix the first one acquired and comes up moving. The seed now applies only where the pace was inherited — a resume, or a start that begins moving — and a fresh start keeps the pace it was given ([#344](https://github.com/Ikolvi/Tracelet/issues/344)).

**FIX**: (Android, iOS) turning logging up no longer destroys the evidence. The always-on lifecycle trace bypasses `logLevel` because it is the record of what the background and killed-state paths did — but it was then pruned by row count alongside everything else, and at `logLevel: verbose` that cap is 2000 rows against a sensor trace emitting several lines a second. A field report's "Session lifecycle" section came back holding a single line after an hour of tracking: the entries were not missing, they had been evicted, and the retention window had collapsed from the configured `logMaxDays` to a couple of minutes. The row cap is now applied per channel, so the chatty one can no longer crowd out the one the report is written to carry ([#318](https://github.com/Ikolvi/Tracelet/issues/318)).

**FIX**: (Android) being near a geofence no longer tells the SDK the device is moving. A fence too small for the OS to resolve is registered inflated and decided in-app, so the transition the OS fires is a *hint* that you are near it — and claiming that hint ran through the same switch a real motion wake-up does, which writes `isMoving = true`, cancels the stationary schedule, and opens a full-rate stream. The reported device had parked itself correctly one line earlier (`accelMoving=false speedMoving=false`) and then streamed 2-second GPS with the indicator lit for the rest of the session, reporting `Is moving: true` on a desk. The pace was inherited from there: every later resume read it, forced the speed machine back to MOVING, and left the coordinator with no stationary decision to make. The wake-up now **borrows** the stream instead — the pace is left exactly as the motion sensors set it, and the loan is bounded: when the window closes the coordinator re-judges the state it is in, parking again if both inputs still say stationary and leaving the stream alone if the device really did move ([#414](https://github.com/Ikolvi/Tracelet/issues/414)). Two supporting corrections ride with it: a session resuming into a live process now takes the coordinator's speed input from the pace machine rather than from the intent — `start()` returns early when the machine is already running, so on that path the machine keeps whatever state it had, which may be slowing or stationary — and the posture reconcile is recorded on the always-on lifecycle channel, because it is the entry that answers "why is GPS still running" and a released build was discarding it at the default `logLevel` ([#414](https://github.com/Ikolvi/Tracelet/issues/414), [#318](https://github.com/Ikolvi/Tracelet/issues/318)).

**FIX**: (Android, iOS) the parked-device GPS drain had two further causes, and on iOS a report could not show either. When the GPS-speed machine reports stationary while the accelerometer still reports motion, the SDK settles the disagreement with the last resolved speed: near zero means the accelerometer is reading hand tremor on a still device, and it is overruled. That override is itself a motion transition, and whenever the speed input was already stationary it is the one carrying the whole stop decision — it was computed and then discarded, and the speed transition behind it then changed nothing, so the stream was never told. The speed input reaches that stale state on its own: it lives for the whole process, and only a session *starting stationary* re-asserted it, so any session that parked handed the next started-moving one a value it could never clear — leaving that session's continuous GPS hanging on the accelerometer alone. On iOS the transition was also invisible: the motion pipeline parks by stopping location updates without ending the session, which never runs the code that records `location stream: continuous updates stopping`, so a released app's report could not say when — or whether — GPS was parked, the one line an Android report has always carried. And iOS decided whether a stream was live from a flag that stays true on a parked engine, which could wedge the same belief in the opposite direction and swallow the next wake-up.
