...
class Tracelet {
  ...
  /// Updates only the tracking indicator's content and reposts it, without a
  /// full [setConfig] round-trip.
  ///
  /// [setConfig] merges an entire [Config], so refreshing the notification
  /// text that way means re-sending a payload that can also carry
  /// `http.url`/`headers` — an accidental overwrite there silently breaks
  /// uploads. This method's payload physically cannot express those fields.
  ///
  /// Only the supplied fields change; everything else keeps its persisted
  /// value. On Android the foreground-service notification is reposted (safe
  /// no-op if the service isn't running). On iOS the fields merge into the
  /// Live Activity: [text] updates the body, [title] is immutable on a running
  /// activity (applied on the next activity start), and the call is a logged
  /// no-op if no [LiveActivityConfig] was configured. On Web: no-op.
  ///
  /// [startedAt] is epoch milliseconds (wall clock); with [showTimer] true the
  /// OS itself renders a count-up clock from that instant — no reposts.
  /// [showTimer] true without a startedAt (now or previously persisted) shows
  /// no timer.
  static Future<void> setNotification({
    ...
    final currentIos = _currentConfig.ios;
    final currentLiveActivity = currentIos.liveActivityConfig;
    _currentConfig = _currentConfig.copyWith(
      android: _currentConfig.android.copyWith(
      ...
      ios: currentLiveActivity == null
          ? currentIos
          : currentIos.copyWith(
              liveActivityConfig: LiveActivityConfig(
                title: title ?? currentLiveActivity.title,
                body: text ?? currentLiveActivity.body,
                startedAt: startedAt ?? currentLiveActivity.startedAt,
                showTimer: showTimer ?? currentLiveActivity.showTimer,
              ),
            ),
    );
