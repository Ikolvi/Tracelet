 ...
 *
 * Copies `website/app/<locale>/**` at <git-tag> into `website/app/<locale>/<slug>/`
...
// A slug that collides with a real docs page would shadow it.
if (fs.existsSync(path.join(APP_DIR, 'en', slug))) {
  fail(`app/en/${slug} already exists — pick another slug or delete it first`);
}
...
export default async function ArchivedLayout({ children }: { children: React.ReactNode }) {
  const pageMap = await getPageMap('/${locale}/${slug}')
...
// Hide the archive from the live sidebar. Without this the archived tree shows
// up as a top-level entry in the current docs' navigation.
//
// Only for locales that actually captured the archive: Nextra validates that
// every _meta key resolves to a real page, so adding the key to a locale with
// no archived tree fails the build outright.
for (const locale of capturedLocales) {
  const metaPath = path.join(APP_DIR, locale, '_meta.js');
  if (!fs.existsSync(metaPath)) continue;
  const meta = fs.readFileSync(metaPath, 'utf8');
  if (meta.includes(`"${slug}"`)) continue;
  const entry = `  "${slug}": {\n    "title": "${label} (archived)",\n    "display": "hidden"\n  },\n`;
  const idx = meta.indexOf('{');
  fs.writeFileSync(metaPath, meta.slice(0, idx + 1) + '\n' + entry + meta.slice(idx + 1));
}
...
console.log(`  ✓ hidden from the live sidebar via _meta.js\n`);
