...
<Callout type="info">
**Only needed if you use attestation.** Without it, all other features work normally — `Tracelet.getAttestationToken()` just returns `null` instead of a Play Integrity token. See [Android platform setup](/en/getting-started/platform-setup/android#optional-dependency-play-integrity-device-attestation) for details.
</Callout>
