...
### Showing an Elapsed Timer in the Live Activity

For a bounded session — a ride, a delivery, a shift — the Live Activity can show a clock counting up from a moment you choose:

```dart
final rideStartedAt = DateTime.now();

ios: tl.IosConfig(
  liveActivityConfig: tl.LiveActivityConfig(
    title: 'Ride in progress',
    body: 'Tracking your route to the destination...',
    startedAt: rideStartedAt.millisecondsSinceEpoch,
    showTimer: true,
  ),
)
```

**iOS renders the clock itself** via `Text(timerInterval:countsDown: false)`. Tracelet pushes no periodic updates to advance it, and the count is not restarted by an unrelated refresh. This mirrors the [Android notification timer](/en/getting-started/platform-setup/android#showing-an-elapsed-timer-in-the-notification), and the semantics match: `showTimer: false` is exactly today's behaviour, `showTimer: true` without a `startedAt` shows no clock, a future `startedAt` is clamped when rendered, and the timer only counts up.

<Callout type="warning">
  **Your Widget Extension must declare the fields to draw them.** Because the extension defines its own copy of `TraceletActivityAttributes`, setting `showTimer` in Dart is not enough on its own — `ContentState` needs `startedAt` and `showTimer`, and the view needs a `Text(timerInterval:)`. Both are in the [snippet above](#scenario-5-live-activities-lock-screen--dynamic-island-ui). An extension without them keeps working and shows no clock.
</Callout>

### Refreshing the Live Activity While Tracking (`updateNotification()`)

To change what the Live Activity shows *after* tracking has started, update `liveActivityConfig` via `setConfig()` and then call `Tracelet.updateNotification()`. Since **v3.6.8** this refreshes the Live Activity from the latest config **without restarting the tracking pipeline** — the cross-platform counterpart to refreshing the [Android foreground-service notification](/en/getting-started/platform-setup/android#refreshing-the-notification-while-tracking-updatenotification):
...
public struct TraceletActivityAttributes: ActivityAttributes {
    public struct ContentState: Codable, Hashable {
        ...
        // Optional elapsed-timer fields. Decoded with `decodeIfPresent` so this
        // struct still works against SDK versions that never send them.
    ...
    var body: some WidgetConfiguration {
        ActivityConfiguration(for: TraceletActivityAttributes.self) { context in
            ...
            HStack(spacing: 12) {
                ...
                Spacer()
                // Elapsed timer. iOS ticks this itself — no updates are pushed.
                // `min(..., Date())` clamps a start instant that is in the
                // future, which would otherwise render as a stuck clock.
...
<Callout type="info">
  **Already shipped a Widget Extension?** The `startedAt` / `showTimer` fields on `ContentState` are additive. An extension built before they existed keeps working exactly as it did — Swift's generated decoder ignores keys it does not know about — it simply shows no clock. Copy the two fields and the `init(from:)` above into your struct when you want [the elapsed timer](#showing-an-elapsed-timer-in-the-live-activity).
</Callout>
