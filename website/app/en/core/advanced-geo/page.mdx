...
**Changed in 3.8.0.** Smoothing now runs *before* the location filters, so
it also applies to the **odometer**. Before 3.8.0 it only affected the
`coords` you received — the odometer kept accumulating raw, jagged deltas, so
turning the filter on smoothed your map but left distance over-reported. If you
read `Tracelet.getOdometer()`, expect your recorded distances to drop after
upgrading. That drop is the noise you were previously being charged for.
</Callout>

<Callout type="info">
**Smoothing alone will not fix distance.** The Kalman filter removes jitter, but
the odometer also depends on `odometerAccuracyThreshold` (default `50` m — quite
loose) and `maxImpliedSpeed`. On foot, where GPS noise is large relative to real
movement, those two matter more than the smoother. Rather than tune them by
hand, see [auto-tuning by transport mode](/en/core/driving-safety#transport-mode).
</Callout>

<Callout type="info">
**These thresholds apply at runtime.** `trackingAccuracyThreshold`,
`odometerAccuracyThreshold`, `maxImpliedSpeed` and `useKalmanFilter` take effect
on the next fix when changed through `setConfig()` — no restart needed, and the
odometer keeps its anchor across the change rather than losing a segment. To
confirm what the filters are actually using at any moment, call
`Tracelet.getCurrentLocationTuning()`, which reads back from the location
processor instead of echoing the config you set.
</Callout>
